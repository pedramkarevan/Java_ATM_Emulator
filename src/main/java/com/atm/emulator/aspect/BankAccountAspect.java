...
import com.atm.emulator.controller.AuthRestController;
...
@Aspect
@Component
public class BankAccountAspect extends AuthRestController {
    ...
    @Before(value = "@annotation(com.atm.emulator.aspect.AccountOperation)")
...
public void getAccountOperationInfo(JoinPoint joinPoint) {
    ...
    System.out.println("Account operation value: " + accountOperation.operation());
        ...
        TransactionLogEntity transLog = new TransactionLogEntity();
        transLog.setCustomer(customerRepository.getById((Long.parseLong(joinPoint.getArgs()[0].toString()))));
        transLog.setAmount ( joinPoint.getArgs()[1].toString());
        transLog.setOperation(accountOperation.operation());
        transLog.setInsertTime(LocalDateTime.now());
        transRepo.saveAndFlush(transLog);
}
}